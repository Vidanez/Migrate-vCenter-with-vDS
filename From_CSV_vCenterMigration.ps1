...
#Activate VDS migration
$VDSpresent = $MIGRATION.VDS
...
$folderquestion = new-object -comobject wscript.shell
$intAnswer = $folderquestion.popup("**Please decided whether to export and import folders**

Normally, this step is needed during the first migration for each datacenter but, you may want to run it if folders were created since the last migration run.  Process will take more time depending on number of folders.

Click Yes to export/import the folders or No to not export/import the folders.
",0,"Export and Import Folders?",4)
If ($intAnswer -eq 6) {
  $folderquestion.popup("You decided to export/import folders.")
} else {
  $folderquestion.popup("You decided not to export/import folders.")
}
...
.....Configuring HA settings for cluster $Cluster in vCenter $DestinationvCenter under datacenter $DestinationDatacenter
...
.....Configuring DRS settings for cluster $Cluster in vCenter $DestinationvCenter under datacenter $DestinationDatacenter
...
If ($VDSpresent -match "TRUE") {
    ...
    foreach( $row in $report)
        ...
        New-dvSwitch -dcName $DestinationDatacenter -dvSwName $row.olddvSwitch -baseUplink 1 -nrUplink 1
...
Write-Host 'Press any key to proceed with the next step of migrating the ESX hosts in' $Cluster -ForegroundColor Green
Write-Host " "
$host.UI.RawUI.ReadKey("NoEcho,IncludeKeyDown") | Out-Null
...
Write-Host "
Disconnecting the hosts in " -NoNewline
Write-Host $Cluster -ForegroundColor Green -NoNewline
...
Set-Cluster -Cluster $Cluster -Name $Cluster" - Migrated to https://newvcenter-VCENTER_NAME_HERE/" -Confirm:$false
...
foreach ($vm in $VMFolderLocationImport) { If (($vm.Folder -Like "Discovered virtual machine") -or ($vm.Folder -Like "vm"))  { } else {Move-VM -VM $vm.Name -Destination $vm.Folder -RunAsync}}
...
Write-Host " "
Write-Host -NoNewline "Hit any key to proceed to the migration of the next cluster.  If there is not a next cluster the script will exit. . . . " -ForegroundColor Green
$host.UI.RawUI.ReadKey("NoEcho,IncludeKeyDown") | Out-Null
